                ...
                <div class="card-body p-5">
                    ...
                    <form action="{{route('job.store')}}" method="post">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="setting_type" class="form-select custom-input">
                                        <option value="{{$job->setting_type}}">{{$job->setting_type}}</option>
                                        @foreach (Job::ALLOWED_SETTING_TYPES as $type)
                                            <option value="{{__('settingTypes.$type')}}">
                                                {{__("settingTypes.$type")}}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="duration" class="form-select custom-input">
                                        ...
                                        @foreach (Job::ALLOWED_DURATION_TYPES as $type)
                                            <option value="{{__('durationTypes.$type')}}">
                                                {{__("durationTypes.$type")}}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" class="form-control custom-input" rows="3">{{$job->description}}</textarea>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">
                                {{__('jobCreate.Expectation')}}</label>
                            <textarea name="expectation" class="form-control custom-input" rows="3">{{$job->expectation}}</textarea>
                        </div>

                        <div class="mb-4">
                            <label class="form-label">
                                {{__('jobCreate.Offer')}}</label>
                            <textarea name="offer" class="form-control custom-input" rows="3">{{$job->offer}}</textarea>
                        </div>

                        <div class="mt-4">
                            <button type="submit" class="btn btn-success btn-lg w-100 py-3 shadow-sm">
                                {{__('jobCreate.UPDATE JOB')}}
                            </button>
                        </div>
                    </form>
